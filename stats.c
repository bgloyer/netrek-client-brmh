...
#ifdef ARMY_SLIDER
   sliders[6].var = &(me->p_armies);	/* note -- changed p_armies to int */
#endif				/* ARMY_SLIDER */
...
calibrate_stats()
{
   ...
   /* was multiplied times 1.2, now no excess: a limited poll seems to
      indicate that no one likes the misleading behavior */
   sliders[4].max = 1.0 * ((double) me->p_ship.s_maxwpntemp);
   sliders[4].high_red = .667 * ((double) sliders[4].max);

   sliders[5].max = 1.0 * ((double) me->p_ship.s_maxegntemp);
